...
function bearishIct(
  currentIndex: number,
  opts: { top?: number; bottom?: number; confirmations?: boolean } = {},
): IctContext {
...
it("invalidated primary + valid bearish alternative still produces an actionable short with a pending order", () => {
  const out = detectSignals(
    candles,
    [],
    invalidatedPrimaryWithBearishAlt(),
    bearishIct(candles.length - 1),
    opts,
  );
...
it("without any confirmation the bearish B is watch-only (POTENTIAL_B)", () => {
  const out = detectSignals(
    ...
    [],
    invalidatedPrimaryWithBearishAlt(),
    bearishIct(candles.length - 1, { confirmations: false }),
    opts,
  );
