...
it("Phase 6 — clusters equal highs into a single BSL level", () => {
  ...
  const step = 3600;
  const candles: CandleV2[] = Array.from({ length: 40 }, (_, i) =>
    c(i, t0 + i * step, 100, 101, 99, 100),
  );
  const pivots: PivotV2[] = [
    piv(5, candles[5].time, 110.0, "HIGH"),
...
it("Phase 6 — EQH ignores provisional (unconfirmed) pivots", () => {
  const t0 = 1_700_000_000;
  const step = 3600;
  const candles: CandleV2[] = Array.from({ length: 30 }, (_, i) =>
    c(i, t0 + i * step, 100, 101, 99, 100),
  );
  const pivots: PivotV2[] = [
...
it("Phase 6 — daily timeframe suppresses ASIA_* and SESSION_* levels", () => {
  ...
  const candles: CandleV2[] = Array.from({ length: 5 }, (_, i) =>
    c(i, t0 + i * dayStep, 100, 101, 99, 100),
  );
...
it("Phase 6 — current SESSION_HIGH/LOW is marked provisional", () => {
  ...
  const step = 3600;
  const candles: CandleV2[] = Array.from({ length: 6 }, (_, i) =>
    c(i, t0 + i * step, 100, 101, 99, 100),
  );
