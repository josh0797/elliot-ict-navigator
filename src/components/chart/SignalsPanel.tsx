              ...
              >
                ...
                <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-xs font-mono">
                  ...
                  <span className="text-right">
                    {s.rrToTp1.toFixed(2)} / {s.rrToTp2.toFixed(2)}
                  </span>
                </div>
                <div className="mt-1 text-[10px] font-mono text-muted-foreground">
                  TP1: {s.targets[0]?.reason ?? "—"} · TP2: {s.targets[1]?.reason ?? "—"} · TP3:{" "}
                  {s.targets[2]?.reason ?? "—"}
                </div>
                {s.trigger && (
                  <div
                    className={`mt-1 text-[10px] font-mono ${s.trigger.satisfied ? "text-success" : "text-amber-400"}`}
                  >
                ...
                <div className="mt-2 flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono text-primary">
                    {s.scoreOut100}/100 · {s.grade}
                  </span>
                </div>
                <div className="text-[10px] font-mono text-muted-foreground">
                  canon {Math.round(s.score * 100)}% · ml{" "}
                  {s.mlScore !== null ? Math.round(s.mlScore * 100) + "%" : "—"}
                </div>
    ...
    <div className={`rounded-lg border p-4 ${cls}`}>
      <div className="flex flex-wrap items-center gap-3">
        ...
        {sig && (report.decision === "BUY" || report.decision === "SELL") && (
          <span className={`rounded border px-2 py-0.5 text-[10px] font-mono ${txt}`}>
            {isMarket
              ? "MARKET (ejecutar ahora)"
              : isPending
                ? `ARMADO · ${sig.orderType.replace(/_/g, " ")}`
      ...
      {(report.decision === "BUY" || report.decision === "SELL") && sig && (
        <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-1 text-xs font-mono">
          ...
          <div>
            <span className="text-muted-foreground">TP1 </span>
            <span className="text-success">{pxFmt(sig.tp1)}</span>
          </div>
          <div>
            <span className="text-muted-foreground">RR </span>
            {sig.rrToTp1.toFixed(2)}
          </div>
        </div>
      )}
        ...
        <div className="mt-2 space-y-1 text-xs">
          ...
          {sig?.invalidation?.price != null && (
            <div>
              ...
              <span className="text-destructive">{pxFmt(sig.invalidation.price)}</span>
      ...
      <div className="mt-2 text-[10px] text-muted-foreground">
        Modelo legacy:{" "}
        {sig?.mlScore != null ? `${Math.round(sig.mlScore * 100)}% (diagnóstico)` : "—"}
      </div>
    </div>
  );
}
